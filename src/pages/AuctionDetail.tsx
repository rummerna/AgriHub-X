...
const AuctionDetail = () => {
  ...
  const fetchAuction = useCallback(async () => {
    ...
    if (data) {
      setAuction(data);
      // Get seller name
      const { data: profile } = await supabase.from("profiles_public" as any as 'profiles').select("full_name").eq("user_id", data.seller_id).single();
  ...
  const fetchBids = useCallback(async () => {
    ...
    if (data) {
      ...
      const { data: profiles } = await supabase.from("profiles_public" as any as 'profiles').select("user_id, full_name").in("user_id", userIds);
