...
const ProductDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const { data: profile } = await supabase.from("profiles_public").select("user_id, full_name, avatar_url, bio, country, county, role, verified, verification_status, rating_avg, rating_count, trade_count, currency").eq("user_id", p.user_id).single();
      setSeller(profile);
