...
function getNextSevenDays(): { day: string; date: string }[] {
  const days = [];
...
export function aggregateWeather(county: string): AggregatedWeather {
  ...
  const daily: DailyForecast[] = nextDays.map(({ day, date }, i) => ({
    day,
    date,
  ...
  if (rainConsensus) {
    ...
    const monthNames = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
    const validUntilStr = `${monthNames[tomorrow.getMonth()]} ${tomorrow.getDate()}, ${tomorrow.getFullYear()} 6:00 PM`;
    alerts.push({
      id: "wa1",
      type: "storm",
  ...
  // Pest markers — dates relative to today for freshness
